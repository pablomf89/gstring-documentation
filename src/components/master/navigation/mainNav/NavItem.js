    ...
    const _nodeItem = this.props?.navItem?.id === this.props.currentPage?.id ? this.props?.currentPage : this.props?.navItem;
      ...
      return (
        <>
          ...
          <Link className={`menu-link ${active && "active"} ${isCurrentNav && "current-nav"}`} to={_nodeItem?.url}>
            {_nodeItem?.N}
          </Link>
                ...
                {this.state.navItems &&
                  ...
                  this.state.navItems.map((navItem, i) => {
                    return (
                        ...
                        <NavItem preloadChildren={openSubMenu} key={navItem.id} navItem={navItem} currentNode={this.props.currentNode} currentNodeAncestors={this.props.currentNodeAncestors} />
